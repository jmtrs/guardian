export const es = {
  common: {
    loading: 'Cargando...',
    error: 'Error',
    retry: 'Reintentar',
    cancel: 'Cancelar',
    back: 'Volver',
  },
  errors: {
    networkError: 'Sin conexion con el servidor',
    generic: 'Algo ha ido mal',
    RATE_LIMIT_EXCEEDED: 'Demasiados intentos, espera un momento',
    INVALID_OTP_CODE: 'Codigo incorrecto',
    INVALID_EMAIL: 'Email no valido',
  },
  auth: {
    loginTitle: 'Guardian',
    loginSubtitle: 'Accede con tu email',
    emailLabel: 'Email',
    emailPlaceholder: 'tu@email.com',
    sendCode: 'Enviar código',
    sendingCode: 'Enviando...',
    resendCode: 'Reenviar código',
    resendCountdown: 'Reenviar en {{seconds}}s',
    verifying: 'Verificando...',
    verifyTitle: 'Introduce el código',
    verifySubtitle: 'Te hemos enviado un código de 6 dígitos',
    invalidCode: 'Código incorrecto',
    invalidEmail: 'Email no valido',
    changeEmail: 'Usar otro email',
    logout: 'Cerrar sesión',
  },
  home: {
    title: 'Guardian',
    subtitle: 'Sistema anti-robo · 12V',
    statusArmed: 'ARMADO',
    statusTrip: 'VIAJE AUTORIZADO',
    statusAlert: 'ALERTA',
    statusAcknowledged: 'REVISADO',
    statusWorkshop: 'TALLER',
    tripRequesting: 'SOLICITANDO',
    acknowledge: 'Revisado',
    workshopCountdown: 'Taller · quedan {{time}}',
    startTrip: 'Iniciar viaje',
    endTrip: 'Finalizar viaje',
    events: 'Eventos recientes',
    eventsTitle: 'Historial',
    noEvents: 'Sin eventos',
    noEventsHint: 'Aún no se ha registrado actividad.',
    noDevice: 'Sin dispositivos. Introduce el codigo de tu Guardian para reclamarlo.',
    noDeviceTitle: 'Sin dispositivos',
    noDeviceHint: 'Introduce el codigo de emparejamiento de tu Guardian para reclamarlo.',
    claimPlaceholder: 'XXXX-XXXX',
    claimAction: 'Reclamar',
    claimError: 'Codigo invalido o caducado.',
    lastSeen: 'Ultimo contacto',
    lastSeenNever: 'Nunca',
    battery: 'Energia',
    powerVehicle: 'vehiculo',
    powerReserve: 'reserva',
    locate: 'Actualizar ubicacion',
    locateWithFix: 'Actualizar · {{time}}',
    locateWaiting: 'Esperando fix...',
    locateUpdated: 'Actualizado {{time}}',
    locateExpired: 'Sin respuesta - reintentar',
    settings: 'Ajustes',
    mapNoPosition: 'Sin posicion aun',
  },
  settings: {
    title: 'Ajustes',
    appearance: 'Apariencia',
    theme: 'Tema',
    themeHint: 'Paleta de color del panel',
    customAccent: 'Acento personalizado',
    customAccentHint: 'Desliza para elegir tu color',
    language: 'Idioma',
    session: 'Sesion',
    done: 'Listo',
  },
  map: {
    title: 'Ubicacion',
    addressUnknown: 'Direccion desconocida',
    trailFromAlert: 'Rastro desde la alerta',
    trailFromTrip: 'Rastro del viaje',
    thirdPartyNotice:
      'Mapa y direcciones vía OpenStreetMap. Al abrir Google Maps o Waze envías las coordenadas a un tercero.',
    noPositionTitle: 'Sin posicion',
    noPositionHint: 'El dispositivo aun no ha reportado una posicion GNSS.',
  },
  events: {
    kindSuspectedMovement: 'Movimiento sospechoso',
    kindBatteryLow: 'Bateria baja',
    kindPowerLost: 'Alimentacion cortada',
    kindHeartbeat: 'Latido',
    kindGnssFix: 'Posicion GNSS',
  },
};

export type Translation = typeof es;
